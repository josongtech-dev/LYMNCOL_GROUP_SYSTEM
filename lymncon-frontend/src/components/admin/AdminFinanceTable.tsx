...
import { CheckCircle2, Eye, Flag, Mail, Phone, ShieldAlert, Star, Trash2 } from 'lucide-react'
import {
  BRAND_COLORS,
  LymnconActionMenu,
  LymnconTable,
...
const defaultActionOptions: LymnconActionItem[] = [
  { label: 'Assign Project', value: 'assign-project', icon: <Icon as={CheckCircle2} boxSize={3.5} /> },
...
export function AdminFinanceTable({
  records,
  columns = ['Officer / Team', 'Role', 'Successful Releases', 'Rating', 'Contact', 'Status', 'Actions'],
  actionOptions = defaultActionOptions,
}: AdminFinanceTableProps) {
  const getStatusBadge = (status: FinanceRecord['status']) => {
    const statusMap: Record<FinanceRecord['status'], { color: string }> = {
      Active: { color: 'green' },
